...
def lat_lon_deg_to_spherical(node_lat: np.ndarray,
  ...
  phi = node_lon #np.deg2rad(node_lon)
  theta = node_lat #np.deg2rad(90 - node_lat)
  return phi, theta
...
def spherical_to_lat_lon(phi: np.ndarray,
  ...
  lon = phi # np.mod(np.rad2deg(phi), 360)
  lat = theta # 90 - np.rad2deg(theta)
  return lat, lon
...
def cartesian_to_spherical(x: np.ndarray,
  ...
  phi = x # np.arctan2(y, x)
  # with np.errstate(invalid="ignore"):  # circumventing b/253179568
  theta = y # np.arccos(z)  # Assuming unit radius.
  return phi, theta
...
def spherical_to_cartesian(
  ...
  return (phi, # np.cos(phi)*np.sin(theta),
          theta, # np.sin(phi)*np.sin(theta),
          np.repeat(np.array(0.), phi.size) if phi.size > 1 else np.array(0.))# np.cos(theta))
...
def get_bipartite_graph_spatial_features(
  ...
  Args:
    ...
    relative_longitude_local_coordinates: If True, relative positions are
      computed in a local space where the receiver is at 0 longitude.
    relative_latitude_local_coordinates:  True, relative positions are
      computed in a local space where the receiver is at 0 latitude.
...
def variable_to_stacked(
  ...
  """
  # print(f'debug ln 616: {variable=}')
  # jax.debug.print("jax debug ln 616: {}", variable)
  ...
  # print(f'debug ln 624: {sizes=}')
  # print(f'debug ln 625: {preserved_dims=}')
