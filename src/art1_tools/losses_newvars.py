...
from . import xarray_tree_newvars
import numpy as np
...
import jax
def weighted_mse_coast_proximity(
    predictions: xarray.Dataset,
    targets: xarray.Dataset,
    coast_prox_weights: xarray.DataArray,
    per_variable_weights: Mapping[str, float],
  ) -> LossAndDiagnostics:
  
  def loss(prediction, target, coast_prox_weights):
    loss = (prediction - target)**2
    # jax.debug.print("debug1: 🤯 {loss.data} 🤯", loss=target)
    # jax.debug.print("debug2: 🤯 {loss.data} 🤯", loss=prediction)
    #jax.debug.print("debug2: 🤯 {loss.data} 🤯", loss=coast_prox_weights)

    if coast_prox_weights is None:
      loss *= normalized_latitude_weights(target).astype(loss.dtype)
    else:
      loss *= coast_prox_weights.astype(loss.dtype)
    # land to nan 
    loss = loss.where(coast_prox_weights != 0).astype(loss.dtype)
    #jax.debug.print("debug3: 🤯 {loss.data} 🤯", loss=loss)
    #jax.debug.print("debug3: 🤯 {loss.data.shape} 🤯", loss=loss)

    if 'level' in target.dims:
      loss *= normalized_level_weights(target).astype(loss.dtype)
    return _mean_preserving_batch(loss)

  losses = xarray_tree_newvars.map_structure(loss, predictions, targets, coast_prox_weights)
  #jax.debug.print("debug per batch: 🤯 {loss.analysed_sst.data} 🤯", loss=losses)
  
  # global var1
  # var1 = (losses, per_variable_weights)
...
def weighted_mse_per_level(
  ...
  def loss(prediction, target):
    #global loss
    loss = (prediction - target)**2
    # jax.debug.print("debug: 🤯 {loss.data} 🤯", loss=loss)
    loss *= normalized_latitude_weights(target).astype(loss.dtype)
    #global lossw
    #lossw = loss
    #jax.debug.print("debug per weights: 🤯 {loss.data} 🤯", loss=loss)
    # [02-04-2023]: Esta linea da NAN si los datos solo tienen un nivel
    #               por lo que se elimina la dim de level en los datos 
    #               de entrada.
    if 'level' in target.dims:
      loss *= normalized_level_weights(target).astype(loss.dtype)
    return _mean_preserving_batch(loss)
  # jax.debug.print('# debug losses predcitions: {}', predictions.analysed_sst)
  #global losses
  losses = xarray_tree_newvars.map_structure(loss, predictions, targets)
  #jax.debug.print("debug per batch: 🤯 {loss.analysed_sst.data} 🤯", loss=losses)
...
def sum_per_variable_losses(
  ...
  # global var1
  # var1 = (weights, per_variable_losses)
...
def normalized_latitude_weights(data: xarray.DataArray) -> xarray.DataArray:
  ...
  * Latitude values that fall exactly at the poles.
  ...
  #w = weights / weights.mean(skipna=False)
  #jax.debug.print("weights: 🤯 {weights.data} 🤯", weights=w)
