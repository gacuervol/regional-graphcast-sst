...
def grads_fn(params, state, model_config, task_config, inputs, targets, forcings, weights):
    ...
    if weights is not None:
        assert (len(weights.dims) == 2), 'weights must have size dim of 2'
...
def train_step(params: dict, grads: dict, learning_rate: float,
    ...
    if next(iter(next(iter(params.values())).values())).ndim == 2:
    ...
    if next(iter(next(iter(grads.values())).values())).ndim == 2:
...
def gen_autoregressive_step_schedule(iterations: int, num_autorreg_steps: int) -> Iterator[int]:
    """
    Generate a schedule of autoregressive steps for a given number of iterations.

    Parameters:
        iterations (int): The total number of iterations.
        num_autorreg_steps (int): The number of autoregressive steps.

    Returns:
        Iterator[int]: An iterator yielding the autoregressive step for each iteration.

    Example:
        # Generate an autoregressive step schedule for 100 iterations with 5 autoregressive steps
        schedule = gen_autoregressive_step_schedule(100, 5)
        for step in schedule:
            print(step)
    """
    # Calculate the interval between each autoregressive step
    interval = iterations // num_autorreg_steps
    # Initialize the autoregressive step counter
    autreg_init = 0
    # Generate the autoregressive step schedule as a generator expression
    autoregressive_step_schedule = ((autreg_init := autreg_init + 1) 
                                    if (it % interval) == 0 and autreg_init < num_autorreg_steps else autreg_init 
                                    for it in range(0, iterations + 1))
    return autoregressive_step_schedule

def get_n_params_n_vars(params: dict, 
    ...
    n_vars = len(train_inputs.data_vars) + len(train_targets.data_vars) + len(train_forcings.data_vars)  
   
    return n_params, n_vars
...
def main(grid_hyperparam: dict, mask_filter: str | None, autreg_steps: int, lr: float, fast_test: bool, n_test: int, params_train_name: str):
    fast_test = fast_test 
    n_nb_test = n_test
    hyper_results = {}
    ...
    global json_path 
    json_path = (path_logs_linux if sys.platform.startswith("linux") else path_logs) + f"results_{n_nb_test}_fine.json" 
    f = open((path_logs_linux if sys.platform.startswith("linux") else path_logs) 
             + f"logs_train_{n_nb_test}nb_fine.txt", "w")
    cycle, M_i, msg_steps, latent_size, batch_size = np.concatenate(list(grid_hyperparam.values()))
    params_path = (f'/home/{user_linux}/Doctorado/params/GraphCast/' 
    ...
    with open(params_path, 'rb') as fp:
        ckpt = checkpoint.load(fp, graphcast_newvars.CheckPoint)
    ...
    # Pmap grads
    def pmap_grads_wrapper(n_params, n_vars, params, state, inputs, targets_template, forcings, weights):
        @functools.partial(xarray_jax_newvars.pmap, dim='device', in_axes=((0,) * n_params) + ((0,) * n_vars) + (() if weights is None else (0,))) #---> if weights
        ...
        return pmap_grads(params, state, inputs, targets_template, forcings, weights)
    # Pmap loss
    def pmap_loss_wrapper(n_params, n_vars, params, state, rng, inputs, targets_template, forcings, weights):
        @functools.partial(xarray_jax_newvars.pmap, dim='device', in_axes=((0,) * n_params) + (None,) +  ((0,) * n_vars) + (() if weights is None else (0,))) #---> if weights
        ...
        return pmap_loss(params, state, rng, inputs, targets_template, forcings, weights)

    # Pmap apply forward
    def pmap_forward_wrapper(n_params, n_vars, params, state, rng, inputs, targets_template, forcings): 
        @functools.partial(xarray_jax_newvars.pmap, dim='device', in_axes=((0,) * n_params) + (None,) +  ((0,) * n_vars))
        def pmap_forward(params, state, rng, inputs, targets_template, forcings):
            ...
            return predictions
        return pmap_forward(params, state, rng, inputs, targets_template, forcings)         
    ...
    for cycle, M_i, msg_steps, latent_size, batch_size in itertools.product(*grid_hyperparam.values()):
        params_path_fine = ((path_logs_linux if sys.platform.startswith("linux") else path_logs) 
                        + f"params_{n_nb_test}_fine_cycle-{cycle}_M_i-{M_i}_msg_steps-{msg_steps}_latent_size-{latent_size}_batch_size-{batch_size}.npz") 
        try:
            # Write hyperparams
            ...
            val_example_batch = load_data.val_example_batch
            """ model_config, _, state = data_model_config(cycle=cycle, M_i=M_i,
                                                msg_steps=msg_steps, 
            ...
            # Model config due to the hyperparams 
            print(model_config)
            ...
            #autoregressive_step_schedule = gen_autoregressive_step_schedule(2*2, 2) # Fast test
            autoregressive_step_schedule = (gen_autoregressive_step_schedule(2*2, 2) 
                                            if fast_test else gen_autoregressive_step_schedule(transition_steps_cos, autreg_steps))
            global_it = 0 
            # Fill 
            #for e in range(2): # Fast test
            #for e in range(2) if fast_test else range(2): # Residual batch debugging
            for e in range(2) if fast_test else range(cycle):
                try:
                    # Train:
                    ...
                    #train_batch_gen = itertools.islice(train_batch_gen, 696, 712) # Residual batch debugging
                    ...
                    #for _ in range(2) if fast_test else range(2): # Residual batch debugging
                    for _ in range(2) if fast_test else range(iter_per_epoch):

                        # Trace number of iterations across epochs
                        # i += len_batch * e
                        # t+1 number of days ahead
                        train_steps = next(autoregressive_step_schedule)#i_train_batch.sizes['time'] - n_days_input #12
                        if f'{train_steps}_autoreg' not in hist_RMSE_val['hist_RMSE']:
                            hist_RMSE_val['hist_RMSE'][f'{train_steps}_autoreg'] = {'mean': [], 'std': []}
                            hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg'] = {'mean': [], 'std': []}
                        print(datetime.now().isoformat(sep=" "))
                        print(f"\tautoregressive step schedule: {train_steps}")
                        f.write(datetime.now().isoformat(sep=" "))
                        f.write(f"\tautoregressive step schedule: {train_steps}\n")
                        ...
                        try:
                            ...
                            train_inputs, train_targets, train_forcings = [xr.concat([device_batch_data_train[f'device_{i}'][j] 
                                                                ...
                                                                dim='device') 
                                                        for j in range(3)]
                            if len(next(iter(next(iter(params.values())).values())).shape) == 1:
                                params = jax.device_put_replicated(params, jax.local_devices())
                                #params = params_to_device(params, n_devices)
                                assert len(next(iter(next(iter(params.values())).values())).shape) == 2, 'Los params no quedaron bien dimensionados para cada GPU'

                            # Si los parámetros no están inicializados, inicializa los parámetros y el estado.
                            assert params != None, 'Phase 3: autoregressive train needs the params from Phase 2. Params can not be None'
                            assert len(next(iter(next(iter(params.values())).values())).sharding.device_set) == 8, 'Params are not in the 8 GPU'
                            
                            n_params, n_vars = get_n_params_n_vars(params, 
                                                            train_inputs,
                                                            train_targets,
                                                            train_forcings)

                            loss_B, diagnostics, next_state, grads = pmap_grads_wrapper(n_params, n_vars,
                                                                            params, state,
                                                                            train_inputs,
                                                                            train_targets,
                                                                            train_forcings,
                            ...
                            with open(params_path_fine, 'wb') as fp:
                        ...
                        print(datetime.now().isoformat(sep=" "))
                        print(f"\titeration: {global_it}, \n\tmin-batch (size: {device_batch_data_train['device_0'][0].dims['batch']}), loss each batch: {hist_loss_train['hist'][f'{e}_epoch']}")
                        f.write(datetime.now().isoformat(sep=" "))
                    ...
                    for device_batch in itertools.islice(val_batch_gen, 0, 2) if fast_test else range(val_iters_per_device):
                        if device_batch < (val_iters_per_device - 1):
                            ...
                            loss_val_B, diagnostics_val = pmap_loss_wrapper(n_params, n_vars, params, state, 
                            ...
                            predictions = pmap_forward_wrapper(n_params, n_vars, params, state, 
                                                        jax.random.PRNGKey(0),
                                                        val_inputs,
                                                        val_targets * np.nan,
                                                        val_forcings)
                        ...
                        for n in range(residual_val) if (device_batch >= (val_iters_per_device - 1)) else range(n_devices):
                            wRMSE_mean_val, wRMSE_std_val = metrics.RMSE_mean_std(predictions['analysed_sst'].isel(device=n), 
                                                                                    ...
                                                                                    land_mask=land_mask,
                                                                                    weights_type='cos')
                            if np.ndim(wRMSE_mean_val.data) > 2:
                                hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['mean'] += wRMSE_mean_val.data.__array__().tolist()
                                hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['std'] += wRMSE_std_val.data.__array__().tolist()
                            else:
                                hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['mean'].append(wRMSE_mean_val.data.__array__().tolist())
                                hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['std'].append(wRMSE_std_val.data.__array__().tolist())
                        
                            RMSE_mean_val, RMSE_std_val = metrics.RMSE_mean_std(predictions['analysed_sst'].isel(device=n), 
                                                                                    val_targets['analysed_sst'].isel(device=n),
                                                                                    land_mask=land_mask, 
                                                                                    weights_type=None)
                            if np.ndim(RMSE_mean_val.data) > 2:
                                hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['mean'] += RMSE_mean_val.data.__array__().tolist()
                                hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['std'] += RMSE_std_val.data.__array__().tolist()
                            else:
                                hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['mean'].append(RMSE_mean_val.data.__array__().tolist())
                                hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['std'].append(RMSE_std_val.data.__array__().tolist())
                    ...
                    if np.ndim(hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['mean']) > 2:
                        hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg']['mean'] = np.mean(hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['mean'], axis=0).tolist()
                        hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg']['std'] = np.mean(hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['std'], axis=0).tolist()
                        hist_RMSE_val['hist_RMSE'][f'{train_steps}_autoreg']['mean'] = np.mean(hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['mean'], axis=0).tolist()
                        hist_RMSE_val['hist_RMSE'][f'{train_steps}_autoreg']['std'] = np.mean(hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['std'], axis=0).tolist()
                    else:
                        hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg']['mean'] = hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['mean']
                        hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg']['std'] = hist_RMSE_val['hist_wRMSE'][f'{e}_epoch']['std']
                        hist_RMSE_val['hist_RMSE'][f'{train_steps}_autoreg']['mean'] = hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['mean']
                        hist_RMSE_val['hist_RMSE'][f'{train_steps}_autoreg']['std'] = hist_RMSE_val['hist_RMSE'][f'{e}_epoch']['std']
                        
                    assert np.shape(hist_RMSE_val['hist_wRMSE'][f'{train_steps}_autoreg']['mean']) == (val_example_batch.dims['lat'], val_example_batch.dims['lon']), f"RMSE matrix must be of shape: ({val_example_batch.dims['lat']}, {val_example_batch.dims['lon']})"
                    
                    hist_RMSE_val['wRMSE']['mean'] = np.mean([hist_RMSE_val['hist_wRMSE'][f'{aut_stp}_autoreg']['mean']
                                                                for aut_stp in range(1, train_steps + 1)], axis=0).tolist()
                    hist_RMSE_val['wRMSE']['std'] = np.mean([hist_RMSE_val['hist_wRMSE'][f'{aut_stp}_autoreg']['std']
                                                                for aut_stp in range(1, train_steps + 1)], axis=0).tolist()
                    hist_RMSE_val['RMSE']['mean'] = np.mean([hist_RMSE_val['hist_RMSE'][f'{aut_stp}_autoreg']['mean']
                                                                for aut_stp in range(1, train_steps + 1)], axis=0).tolist()
                    hist_RMSE_val['RMSE']['std'] = np.mean([hist_RMSE_val['hist_RMSE'][f'{aut_stp}_autoreg']['std']
                                                                for aut_stp in range(1, train_steps + 1)], axis=0).tolist()
            ...
            for device_batch in itertools.islice(test_batch_gen, 0, 2) if fast_test else range(test_iters_per_device):
                if device_batch < (test_iters_per_device - 1):
                    ...
                    test_inputs, test_targets, test_forcings = [xr.concat([device_batch_data_test[f'device_{i}'][j] 
                                                        ...
                                                        dim='device') 
                                                    for j in range(3)]
                    loss_test_B, diagnostics_test = pmap_loss_wrapper(n_params, n_vars, params, state, 
                                                            jax.random.PRNGKey(0),
                                                            test_inputs,
                                                            test_targets,
                                                            test_forcings,
                ...
                for n in range(residual_test) if (device_batch >= (test_iters_per_device - 1)) else range(n_devices):
                    ...
                    hist_RMSE_test['hist_wRMSE']['mean'] += wRMSE_mean_test.data.__array__().tolist()
                    hist_RMSE_test['hist_wRMSE']['std'] += wRMSE_std_test.data.__array__().tolist()
                    ...
                    hist_RMSE_test['hist_RMSE']['mean'] += RMSE_mean_test.data.__array__().tolist()
                    hist_RMSE_test['hist_RMSE']['std'] += RMSE_std_test.data.__array__().tolist()
            ...
            hist_RMSE_test['wRMSE']['mean'] += np.mean(hist_RMSE_test['hist_wRMSE']['mean'], axis=0).tolist()
            hist_RMSE_test['wRMSE']['std'] += np.mean(hist_RMSE_test['hist_wRMSE']['std'], axis=0).tolist()
            hist_RMSE_test['RMSE']['mean'] += np.mean(hist_RMSE_test['hist_RMSE']['mean'], axis=0).tolist()
            hist_RMSE_test['RMSE']['std'] += np.mean(hist_RMSE_test['hist_RMSE']['std'], axis=0).tolist()
    ...
    shutil.copy(params_path_fine, destination)
...
if __name__ == "__main__":
    ...
    parser.add_argument("mask_filter", 
                        ...
                        type=str)
    parser.add_argument("autreg_steps", 
                        help="number of autoregresive steps e.g. 10",
                        type=int)
    parser.add_argument("lr", 
                        help="Learning rate e.g. 3e-07",
                        type=float)
    ...
    parser.add_argument("n_test", 
                        ...
                        type=int)
    parser.add_argument("params_train_name", 
                        help="File name of the train params. It is not the path e.g. params_41_cycle-10_M_i-3_msg_steps-4_latent_size-32_batch_size-8.npz",
                        type=str)
    ...
    hyper_results = main(grid_hyperparam, args.mask_filter, args.autreg_steps, args.lr, args.fast_test, args.n_test, args.params_train_name)
    # Save results
    with open(json_path, "w") as js:
        # Write the dictionary to the file
        json.dump(hyper_results, js, indent=4)