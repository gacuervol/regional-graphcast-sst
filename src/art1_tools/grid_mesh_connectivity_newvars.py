...
def _grid_lat_lon_to_coordinates(
  ...
  phi_grid, theta_grid = np.meshgrid(
      grid_longitude, # np.deg2rad(grid_longitude),
      grid_latitude # np.deg2rad(90 - grid_latitude)
      )
  ...
  return np.stack(
      [phi_grid, # np.cos(phi_grid)*np.sin(theta_grid),
       theta_grid, # np.sin(phi_grid)*np.sin(theta_grid),
       np.zeros(phi_grid.shape)], axis=-1) #np.cos(theta_grid)], axis=-1)
