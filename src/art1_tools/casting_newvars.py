...
class Bfloat16Cast(predictor_base_newvars.Predictor):
  """Wrapper that casts all inputs to bfloat16 and outputs to targets dtype."""

  def __init__(self, predictor: predictor_base_newvars.Predictor, enabled: bool = True):
    """Inits the wrapper.
  ...
  def loss(self,
    ...
    with bfloat16_variable_view():
      loss, scalars = self._predictor.loss(
        *_all_inputs_and_weights_to_bfloat16(inputs, targets, forcings, weights), **kwargs)
  ...
  def loss_and_predictions(  # pytype: disable=signature-mismatch  # jax-ndarray
      ...
      weights: xarray.DataArray,
      **kwargs,
      ) -> Tuple[predictor_base_newvars.LossAndDiagnostics,
                 xarray.Dataset]:
    if not self._enabled:
      return self._predictor.loss_and_predictions(inputs, targets, forcings, weights, # pytype: disable=bad-return-type  # jax-ndarray
                                                  **kwargs)
...
def _all_inputs_and_weights_to_bfloat16(
    inputs: xarray.Dataset,
    targets: xarray.Dataset,
    forcings: xarray.Dataset,
    weights: xarray.DataArray,
  ...
  return (inputs.astype(jnp.bfloat16),
          jax.tree_map(lambda x: x.astype(jnp.bfloat16), targets),
          forcings.astype(jnp.bfloat16),
          weights.astype(jnp.bfloat16) if weights is not None else weights)
