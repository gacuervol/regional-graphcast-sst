...
def pmap(fn: Callable[..., Any],
         ...
         backend: ... = None,
         in_axes: tuple[int] = (0),
         out_axes: tuple[int] = (0),
         static_broadcasted_argnums: ... = (),) -> Callable[..., Any]:
...
try:
  ...
  jax.tree_util.register_pytree_node(
    ...
    # Register other xarray types as needed
except ValueError as e:
    if "Duplicate custom PyTreeDef" in str(e):
      # Registration already happened, ignore the error
      print('Warning: Registration already happened, ignore the error')
      pass
    else:
      raise e  # Raise other unexpected ValueErrors """