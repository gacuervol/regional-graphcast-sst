...
def normalize(values: xarray.Dataset,
  ...
  def normalize_array(array):
    ...
    return array
  return xarray_tree_newvars.map_structure(normalize_array, values)
  ...
  def unnormalize_array(array):
    ...
    if locations is not None:
      if array.name in locations:
        ...
        logging.warning('No normalization location found for %s', array.name)
    return array
  return xarray_tree_newvars.map_structure(unnormalize_array, values)


class InputsAndResiduals(predictor_base_newvars.Predictor):
  ...
  def __call__(self,
    ...
    return xarray_tree_newvars.map_structure(
  ...
  def loss(self,
    ...
    norm_inputs = normalize(inputs, self._scales, self._locations)
    norm_forcings = normalize(forcings, self._scales, self._locations)
    norm_target_residuals = xarray_tree_newvars.map_structure(
  ...
  def loss_and_predictions(  # pytype: disable=signature-mismatch  # jax-ndarray
    ...
    (loss, scalars), norm_predictions = self._predictor.loss_and_predictions(
        norm_inputs, norm_target_residuals, forcings=norm_forcings, weights=weights, **kwargs)
    predictions = xarray_tree_newvars.map_structure(
        lambda pred: self._unnormalize_prediction_and_add_input(inputs, pred),
        norm_predictions)
