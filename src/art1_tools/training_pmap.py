...
SST_SAT_with_nan = data_loader.SST_SAT_with_nan
...
def get_w_land(ds: xr.Dataset, var_name: str):
    bool_mask = np.isnan(ds[var_name].data)

    return np.where(bool_mask, 0, 1)

def get_n_params_n_vars(
    params: dict, 
...
def main(grid_hyperparam: dict, mask_filter: str, fast_test: bool, n_test: int):
    ...
    elif mask_filter == 'land':
        weights = get_w_land(SST_SAT_with_nan, 'analysed_sst')
    ...
    for cycle, M_i, msg_steps, latent_size, batch_size in itertools.product(*grid_hyperparam.values()):
        ...
        try:
            ...
            train_batch_gen, val_batch_gen, _ = data_loader()
            train_example_batch = data_loader.train_example_batch
            model_config, params, state = data_model_config(
                cycle=cycle, 
                M_i=M_i,
                msg_steps=msg_steps, 
            ...
            # Pmap init forward
            def pmap_init_wrapper(n_vars, rng, inputs, targets_template, forcings):
                @functools.partial(xarray_jax_newvars.pmap, dim='device', in_axes=(None,) + ((0,) * n_vars))
                def pmap_init(
                    rng,
                    inputs,
                    targets_template,
                    ...
                    return params, state
                return pmap_init(rng, inputs, targets_template, forcings)
            
            print(model_config)
            f.write(f"{model_config=}\n")
            # Train schedule
            n_devices = len(jax.devices())
            residual_train = 1 if (train_batch_size % n_devices) != 0 else 0  
            train_iters_per_device = ((train_batch_size // n_devices) + residual_train)
            transition_steps_cos = round(train_iters_per_device * cycle)
            ...
            for e in range(2) if fast_test else range(cycle): # range(2):
                try:
                    ...
                    # Set batch gen
                    train_batch_gen, _, _ = data_loader()
                    ...
                    for device_batch in itertools.islice(train_batch_gen, 0, 2) \
                    if fast_test else range(train_iters_per_device):
                        if device_batch < (train_iters_per_device - residual_train):
                    #for _ in range(2) if fast_test else range(2): # Residual batch debugging
                            ...
                            device_batch_data_train = {
                                f'device_{i}': data_utils_newvars.extract_inputs_targets_forcings(
                            ...
                            train_inputs, train_targets, train_forcings = [
                                xr.concat(
                                    [device_batch_data_train[f'device_{i}'][j] 
                                        for i in range(n_devices)],
                                    dim='device',
                                    ) 
                                for j in range(3)
                            ...
                            if params is None:
                                    params, state = pmap_init_wrapper(
                                        n_vars, 
                            ...
                            n_params, _ = get_n_params_n_vars(
                                params, 
                                train_inputs,
                                train_targets,
                                train_forcings,
                            ...
                            loss_B, diagnostics, next_state, grads = pmap_grads_wrapper(
                                n_params, 
                                n_vars,
                                params, 
                                state,
                            ...
                            # accumulate loss of each batch of this iteration
                            sum_loss_train += float(sum(loss_B))
                            ...
                            ckpt = graphcast_newvars.CheckPoint(
                                params=params,
                            ...
                            residual_batch_loss = []
                            for batch_device in range(residual_train):
                                ...
                                residual_train_targets.append(train_targets)
                                # Learning rate
                                lr = cosine_schedule(global_it)
                                # For residual case
                                if type(next(iter(next(iter(params.values())).values()))) is not np.ndarray:
                                    assert len(next(iter(next(iter(params.values())).values())).sharding.device_set) == 8, 'Params are not in GPU'
                                    params = jax.device_get(params)
                                    assert type(next(iter(next(iter(params.values())).values()))) == np.ndarray, 'Params are not in CPU'
                                    flat_params, tree_params = jax.tree_util.tree_flatten(params)
                                ...
                                grads_fn_jitted = (with_params(with_configs(grads_fn)))
                                loss_B, diagnostics, next_state, grads = grads_fn_jitted(
                                    inputs=jax.device_put(train_inputs, jax.devices(backend='cpu')[0]),
                    ...
                    mean_epoch_loss_train = sum_loss_train / (2 * 8 if fast_test else train_batch_size)
                    hist_loss_train['loss'].append(mean_epoch_loss_train)
                    ...
                    for device_batch in itertools.islice(val_batch_gen, 0, 2) if fast_test else range(val_iters_per_device):#val_batch_gen:
                        if device_batch < (val_iters_per_device - residual_val):
                            val_steps = train_steps #i_val_batch.sizes['time'] - n_days_input 
                            device_batch_data_val = {
                                f'device_{i}': data_utils_newvars.extract_inputs_targets_forcings(
                                    next(val_batch_gen), 
                            ...
                            val_inputs, val_targets, val_forcings = [
                                xr.concat(
                                    [device_batch_data_val[f'device_{i}'][j] 
                                     for i in range(n_devices)],
                                    dim='device',
                                    ) 
                                for j in range(3)
                            ...
                            loss_val_B, diagnostics_val = pmap_loss_wrapper(
                                n_params, 
                                n_vars, 
                                params, 
                                state, 
                            ...
                            print(':: ¡Residual device validation batch reached!')
                            residual_preds = []
                            residual_val_targets = []
                            for batch_device in range(residual_val):
                                ...
                                # accumulate loss of each batch of this iteration
                                if loss_val_B.size == 1:
                                    sum_loss_val += float(loss_val_B)
                                else:
                                    sum_loss_val += float(sum(loss_val_B))
                                # Fill val loss per epoch loss_hyper = [e1_val_loss, e2_val_loss, ...,  en_val_loss]
                                def with_params(fn):
                        ...
                        # land_mask = metrics.get_land_mask_batched(
                        #     SST_SAT_with_nan['analysed_sst'], 
                        #     predictions['analysed_sst'].isel(device=0).transpose('batch', 'time', 'lat', 'lon'),
                        #     ) 
                        for n in range(residual_val) if (device_batch >= (val_iters_per_device - residual_val)) else range(n_devices):
                            evaluator = metrics.ForecastEvaluator(
                                predictions['analysed_sst'].isel(device=n).transpose('batch', 'time', 'lat', 'lon'), 
                                val_targets['analysed_sst'].isel(device=n),
                                land_mask=SST_SAT_with_nan['analysed_sst'],
                                weights_type='cos',
                                )
                            wRMSE_mean_val = evaluator.RMSE()
                            sum_wRMSE_val += wRMSE_mean_val.to_numpy()
                            
                        # If residual batch is fully iterated the validation loop must be breaken
                        if device_batch >= (val_iters_per_device - residual_val):
                            break
                    # Fill RMSE and wRMSE per epoch RMSE_hyper = [e1_RMSE, e2_RMSE, ...,  en_RMSE]
                    mean_epoch_loss_val = sum_loss_val / (2 * 8 if fast_test else val_batch_size)
                    hist_loss_val['loss'].append(mean_epoch_loss_val)
    
                    hist_RMSE_val['wRMSE'].append(
                        (sum_wRMSE_val / (2 if fast_test else val_batch_size)).tolist()
                        )

                    print(datetime.now().isoformat(sep=" "))
                    print(f"\n:: min-batch (size: {device_batch_data_val['device_0'][0].dims['batch']}), \nhist val_loss each epoch: {hist_loss_val['loss']} \nwlatRMSE val: {np.nanmean(hist_RMSE_val['wRMSE'][-1])}, \nACC val: {hist_ACC_val}")
        ...
        hyper_results[f"{cycle=}, {M_i=}, {msg_steps=}, {latent_size=}, {batch_size=}"] = {
