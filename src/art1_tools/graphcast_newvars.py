...
import chex
from . import grid_mesh_connectivity_newvars
from . import icosahedral_mesh_newmesh
from . import losses_newvars
from . import model_utils_newvars
from . import xarray_jax_newvars
from . import predictor_base_newvars
from Graphcast_model import deep_typed_graph_net
from Graphcast_model import typed_graph
...
TASK_SST_NO_FORCING = TaskConfig(
    input_variables=MY_VARS + GENERATED_FORCING_VARS + STATIC_VARS_SST,
    target_variables=MY_VARS,
    forcing_variables=GENERATED_FORCING_VARS,
    pressure_levels=PRESSURE_LEVELS_SST,
    input_duration="48h",
)
TASK_SST_NO_FORCING_weekly = TaskConfig(
    input_variables=MY_VARS + GENERATED_FORCING_VARS + STATIC_VARS_SST,
    target_variables=MY_VARS,
    forcing_variables=GENERATED_FORCING_VARS,
    pressure_levels=PRESSURE_LEVELS_SST,
...
class ModelConfig:
  ...
  lat_data: xarray.DataArray
  lon_data: xarray.DataArray
  divisions: int
...
# def mokey6():
#   return var9
class GraphCast(predictor_base_newvars.Predictor):
  """GraphCast Predictor.
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def __init__(self, model_config: ModelConfig, task_config: TaskConfig):
    ...
    self._meshes = (
        icosahedral_mesh_newmesh.get_hierarchy_of_triangular_meshes_for_sphere(
            splits=model_config.mesh_size, 
            lat_data=model_config.lat_data, 
            lon_data=model_config.lon_data, 
            divisions=model_config.divisions)
            )
    ...
    num_atmospheric_vars = len(
        set(task_config.target_variables) & set(ALL_ATMOSPHERIC_VARS))
    # global var6
    # var6 = (num_surface_vars, task_config, num_atmospheric_vars)
  ...
  def __call__(self,
    ...
    # global var3
    # var3 = (updated_latent_mesh_nodes, latent_grid_nodes)
    output_grid_nodes = self._run_mesh2grid_gnn(
        updated_latent_mesh_nodes, latent_grid_nodes)
    # global var4
    # var4 = output_grid_nodes
  ...
  def loss_and_predictions(  # pytype: disable=signature-mismatch  # jax-ndarray
      ...
      forcings: xarray.Dataset,
      weights: xarray.DataArray,) -> tuple[predictor_base_newvars.LossAndDiagnostics, xarray.Dataset]:
    # Forward pass.
    predictions = self(
        inputs, targets_template=targets, forcings=forcings, is_training=True)
    #jax.debug.print("predictions 🤯 {} 🤯", predictions['analysed_sst'])
    #jax.debug.print("targets 🤯 {} 🤯", targets['analysed_sst'])
    # jax.debug.print("predictions 🤯 {} 🤯", predictions)
    # Compute loss.
    ...
    loss = losses_newvars.weighted_mse_coast_proximity(
        predictions, targets,
        weights, per_variable_weights)
  ...
  def loss(  # pytype: disable=signature-mismatch  # jax-ndarray
      ...
      targets: xarray.Dataset,
      forcings: xarray.Dataset,
      weights: xarray.DataArray,) -> predictor_base_newvars.LossAndDiagnostics:
    #jax.debug.print("dentro de la loss fn🤯 {} 🤯", targets)
  ...
  def _init_mesh_properties(self):
    ...
    mesh_phi, mesh_theta = model_utils_newvars.cartesian_to_spherical(
  ...
  def _init_grid2mesh_graph(self) -> typed_graph.TypedGraph:
    ...
    # global var1 
    # var1 = (self._grid_lat, self._grid_lon, self._finest_mesh, self._query_radius)
    (grid_indices, mesh_indices) = grid_mesh_connectivity_newvars.radius_query_indices(
        grid_latitude=self._grid_lat,
        grid_longitude=self._grid_lon,
        mesh=self._finest_mesh,
        radius=self._query_radius)
    # global var2
    # var2 = (grid_indices, mesh_indices)
  ...
  def _init_mesh_graph(self) -> typed_graph.TypedGraph:
    """Build Mesh graph."""
    merged_mesh = icosahedral_mesh_newmesh.merge_meshes(self._meshes)

    # Work simply on the mesh edges.
    senders, receivers = icosahedral_mesh_newmesh.faces_to_edges(merged_mesh.faces)

    # Precompute structural node and edge features according to config options.
    # Structural features are those that depend on the fixed values of the
    # latitude and longitudes of the nodes.
    assert self._mesh_nodes_lat is not None and self._mesh_nodes_lon is not None
    node_features, edge_features = model_utils_newvars.get_graph_spatial_features(
  ...
  def _init_mesh2grid_graph(self) -> typed_graph.TypedGraph:
    ...
    # mesh triangles.
    (grid_indices,
     mesh_indices) = grid_mesh_connectivity_newvars.in_mesh_triangle_indices(
         grid_latitude=self._grid_lat,
         grid_longitude=self._grid_lon,
         mesh=self._finest_mesh)
  ...
  def _inputs_to_grid_node_features(
    ...
    stacked_inputs = model_utils_newvars.dataset_to_stacked(inputs)
    # [26-04-2024]: fix the keyerror: 'lat'related to no external forcings: radiation TOA
    try:
      stacked_forcings = model_utils_newvars.dataset_to_stacked(forcings)
    except KeyError:
      stacked_forcings = model_utils_newvars.dataset_to_stacked(forcings, preserved_dims=("batch", "lon"))
    ...
    grid_xarray_lat_lon_leading = model_utils_newvars.lat_lon_to_leading_axes(
        stacked_inputs)
    return xarray_jax_newvars.unwrap(grid_xarray_lat_lon_leading.data).reshape(
  ...
  def _grid_node_outputs_to_prediction(
    ...
    grid_xarray_lat_lon_leading = xarray_jax_newvars.DataArray(
        data=grid_outputs_lat_lon_leading,
        dims=dims)
    grid_xarray = model_utils_newvars.restore_leading_axes(grid_xarray_lat_lon_leading)

    # xarray `DataArray` (batch, lat, lon, channels)
    # to xarray `Dataset` (batch, one time step, lat, lon, level, multiple vars)

    return model_utils_newvars.stacked_to_dataset(
...
def _get_max_edge_distance(mesh):
  senders, receivers = icosahedral_mesh_newmesh.faces_to_edges(mesh.faces)
