...
class Predictor(predictor_base_newvars.Predictor):
  ...
  def __init__(
      self,
      predictor: predictor_base_newvars.Predictor,
  ...
  def __call__(self,
    ...
    # Debug
    # global debug_220
    # debug_220 = (one_step_prediction, inputs, scan_variables)
  ...
  def loss(self,
           ...
           **kwargs
           ) -> predictor_base_newvars.LossAndDiagnostics:
    ...
    def one_step_loss(inputs, scan_variables):
      ...
      loss, diagnostics = xarray_tree_newvars.map_structure(
          xarray_jax_newvars.unwrap_data, (loss, diagnostics))
    ...
    (loss, diagnostics) = jax.tree_util.tree_map(
        lambda x: xarray_jax_newvars.DataArray(x, dims=('time', 'batch')).mean(  # pylint: disable=g-long-lambda
            'time', skipna=False),
